...
function wrapCodePromptBase(userPrompt: string, depth = 0): string {
  ...
  return [
    ...
    depthRule,
    "",
    "CRITICAL — .claude/ DIRECTORY WRITE RESTRICTION:",
    "- The Write and Edit tools are BLOCKED on ALL .claude/ paths by Claude Code's internal protection.",
    "- This cannot be overridden by any permission setting or flag. Do NOT attempt Write or Edit on .claude/ paths.",
    "- To create or modify files under .claude/, ALWAYS use Bash with heredoc:",
    "    mkdir -p .claude/skills/my-skill && cat > .claude/skills/my-skill/SKILL.md << 'SKILL_EOF'",
    "    content here",
    "    SKILL_EOF",
    "- This applies to skills, agents, rules, hooks, settings — everything under .claude/.",
    "- If a task asks you to create a skill, agent, or rule, you MUST use Bash heredoc. Never try Write or Edit first.",
    "",
    "SECURITY RULES:",
